...
public class SSHUtils {
	...
	static final Logger logger = Logger.getLogger(SSHUtils.class);
...
//			input = this.getClass().getClassLoader().getResourceAsStream("/config.properties");
			...
			System.out.println("Entering loadProperties :: " + input);
			prop.load(input);
			...
			if(this.tipoDespiegue.equals("1")){
				logger.info("Despliegue para ambiente de desarrollo");
				...
				logger.info("Despliegue para ambiente de produccion");
	...
	public void conectar(){
		logger.info("Entering method: conectar");
		if(this.tipoDespiegue.equals("1")){
			cargaLlavesDev();
		} else if(this.tipoDespiegue.equals("2")){
			cargaLlavesProd();
	...
	public void cargaLlavesDev(){
		...
		logger.info("Comenzando a cargar llaves para desarrollo");
	    try {

	    	JSch jsch = new JSch();
	        String absoluteFilePathPrivatekey = "/";
	        File tmpFileObject = new File("src/resources/privateKeyTomcatDev");
	...
	public void cargaLlavesProd(){
		...
		logger.info("Comenzando a cargar llaves para produccion");
	    try {

	    	JSch jsch = new JSch();
	        String absoluteFilePathPrivatekey = "/";
	        File tmpFileObject = new File("src/resources/privateKeyProdTomcat");
	        if (tmpFileObject.exists() && tmpFileObject.isFile()){
	          absoluteFilePathPrivatekey = tmpFileObject.getAbsolutePath();
	        }
	        jsch.addIdentity(absoluteFilePathPrivatekey, passwordTomcat);
	        ...
	        // Oracle
	        String absoluteFilePathPrivatekeyOracle = "/";
	        File tmpFileObjectOracle = new File("src/resources/privatekeyProdOracle");
	        if (tmpFileObjectOracle.exists() && tmpFileObjectOracle.isFile()){
	        	absoluteFilePathPrivatekeyOracle = tmpFileObjectOracle.getAbsolutePath();
	        }
